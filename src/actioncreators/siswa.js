...
export const filterSiswa = (nama) => {
  return (dispatch) => {
    ...
    axios
      .get("http://localhost:3500/siswa", { params })
...
export const getSiswa = () => {
  return function (dispatch) {
    axios.get("http://localhost:3500/siswa").then((response) => {
      dispatch({
        type: "SISWA_LIST",
        payload: response.data,
      });
...
export const hapusSiswa = (id) => {
  ...
  return function (dispatch) {
    ...
    axios.delete(`http://localhost:3500/siswa/${id}`).then((response) => {
...
export const tambahSiswa = (data) => {
  return (dispatch) => {
    axios.post("http://localhost:3500/siswa", data).then((response) => {
      dispatch({
